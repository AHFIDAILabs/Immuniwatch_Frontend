...
function OverrideModal({
  ...
  const [label, setLabel] = useState<ClassificationLabel>(
    (review.classificationId as { label: ClassificationLabel }).label ??
      "irrelevant",
  );
  ...
  const { mutate, isPending } = useMutation({
    ...
    onError: (err) => {
      toast.error(
        "Override failed",
        apiMessage(err, "Could not submit the override. Please try again."),
      );
    },
  });
...
function DispatchModal({
  ...
  const postId =
    post?._id ?? (typeof review.postId === "string" ? review.postId : "");
  const platformLabel = post?.platform
    ? (PLATFORM_LABELS[post.platform] ?? post.platform)
    : "platform";
  const kbSeed =
    cls?.kbEvidence?.[0]?.snippet ?? cls?.kbEvidence?.[0]?.title ?? "";
  ...
  const [cnSource, setCnSource] = useState<"ml" | "groq" | "template" | null>(
    null,
  );
  const [cnVersions, setCnVersions] = useState<Record<CNVersion, string>>({
    short: "",
    medium: "",
    long: "",
  });
  ...
  const { mutate: deploy, isPending: deploying } = useMutation({
    ...
    onSuccess: () => {
      toast.success(
        "Response saved",
        `Counter-narrative recorded. Copy and post to ${platformLabel}.`,
      );
      setDone(true);
    },
    onError: () => {
  ...
  const { mutate: skip, isPending: skipping } = useMutation({
    ...
    onSuccess: () => {
      toast.info("Skipped", "No counter-narrative will be sent.");
      onClose();
    },
    onError: () => {
      toast.info("Skipped", "Marked as reviewed.");
      onClose();
    },
  });
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
          ...
          <p className="text-sm text-emerald-700">
            Response saved. Copy and post it manually on{" "}
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between gap-3">
        <p className="text-xs text-gray-500 flex-1">
          Edit the counter-narrative below, then <strong>Deploy</strong> to
          record it, or <strong>Skip</strong> if no reply is needed.
        </p>
        {cnLoading && (
...
function ContextStrip({ postId }: { postId: string }) {
  const { data } = useQuery({
    ...
    queryFn: () =>
      api
        ...
        }>("/posts/similar-count", { params: { postId } })
        .then((r) => r.data),
    staleTime: 60_000,
  });
  ...
  const topPlatforms = data.platforms
    ...
    .map(
      (p) =>
        `${PLATFORM_LABELS[p.platform as PostPlatform] ?? p.platform} (${p.count})`,
    )
    .join(" · ");
  ...
  return (
    <div className="flex items-center gap-2 text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5 mb-3">
      ...
      <span>
        <strong>{data.count.toLocaleString()}</strong> similar {data.label}{" "}
        posts in the last 24 h — {topPlatforms || "multiple platforms"}
      </span>
    </div>
  );
}
...
export default function HITLQueue() {
  ...
  const [priorityFilter, setPriorityFilter] = useState<HITLPriority | "all">(
    "all",
  );
  ...
  const { mutate: approve } = useMutation({
    ...
    onSuccess: (approved, id) => {
      ...
      toast.success(
        "Review approved",
        "The post has been marked for dispatch.",
      );
    ...
    onError: (err, id) => {
      ...
      toast.error(
        "Approve failed",
        apiMessage(err, "Could not approve the review. Please try again."),
      );
    },
  });
  ...
  const { mutate: reject } = useMutation({
    ...
    onError: (err, id) => {
      ...
      toast.error(
        "Reject failed",
        apiMessage(err, "Could not reject the review. Please try again."),
      );
    },
  });
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="No pending reviews at this priority level."
        />
      ) : (
        <>
          <div className="space-y-3">
            {reviews.map((review) => {
              ...
              const postId =
                post?._id ??
                (typeof review.postId === "string" ? review.postId : "");
                ...
                >
                  ...
                  {kbEvidence.length > 0 ? (
                    <div className="bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2.5 mb-3 text-xs flex gap-2">
                      ...
                      <span className="text-gray-700">
                        {kbEvidence[0].summary ?? kbEvidence[0].title}
                      </span>
                    </div>
                    ...
                    <div className="bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2.5 mb-3 text-xs flex gap-2">
                      ...
                      <span className="text-gray-500">
                        No knowledge base evidence found — enrich the KB to
                  ...
                  {review.status === "pending" ? (
                    <div className="flex gap-2 flex-wrap">
                      <button
                        onClick={() => {
                          if (!isActing) approve(review._id);
                        }}
                        disabled={isActing}
                      ...
                      {canOverride && (
                        <button
                          onClick={() => {
                            if (!isActing) setOverrideTarget(review);
                          }}
                          disabled={isActing}
      ...
      {dispatchTarget && (
        <Modal
          ...
          title="Review Approved — Dispatch Response"
          size="md"
        >
          <DispatchModal
            review={dispatchTarget}
            onClose={() => setDispatchTarget(null)}
          />
        </Modal>
      )}
